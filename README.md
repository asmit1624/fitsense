...
3. **Come back any day** → it remembers you. Type `reset` to rebuild your profile anytime.
...
This boundary matters: a general-purpose 3B model can confidently get sizing *wrong*. By locking size in code and only asking the LLM for opinions, the blast radius of a model error is limited to subjective advice — not the one number that drives returns.

---

## How the Fit Score works today — honestly

The 0–100 **Fit Score is currently the LLM's holistic judgement**, not a formula. Consequences observed and explainable:

- It **clusters at round numbers** (85, 75) — language models gravitate to "nice" scores.
- It's **repetitive for identical inputs** — same profile + same product = same prompt = same output.
- It's *grounded* (facts and the size decision are injected, not invented) but **not explainable** — you can't ask "why 85 and not 78?"

**This is the top roadmap item:** replace the guessed number with a **computed, weighted score in code** — size match + colour-tone match + style match + budget fit — with the LLM only writing the explanation. Target output: `Size 30/30 · Colour 22/25 · Style 18/25 · Budget 15/20 → 85/100`.

---

## What's been tested

- **Multi-user, multi-device:** full journeys from four separate Telegram accounts — four isolated profiles, correct per-user personalisation, **100% onboarding completion** in testing.
...
- n8n (via Docker, included in compose file)

### Run it

```bash
# 1. Clone the repo
git clone https://github.com/asmit1624/FitSense.git
cd FitSense

# 2. Add your Telegram bot token to docker-compose.yml

# 3. Start n8n + Postgres
docker compose up -d

# 4. Load the database
docker exec -i fitsense-postgres psql -U fitsense -d fitsense < db/01_schema.sql
docker exec -i fitsense-postgres psql -U fitsense -d fitsense < db/02_seed.sql
docker exec -i fitsense-postgres psql -U fitsense -d fitsense < db/03_add_onboarding_columns.sql

# 5. Pull and warm the AI model
ollama pull qwen2.5:3b
ollama run qwen2.5:3b "say ok"

# 6. Start the Cloudflare tunnel (gives your laptop a public address)
cloudflared tunnel --url http://localhost:5678
# Copy the generated URL → paste into WEBHOOK_URL in docker-compose.yml → docker compose up -d

# 7. Import workflows into n8n
# Open http://localhost:5678 → Import workflows from the /workflows folder
# Publish the Router workflow
```

### Daily startup (tunnel resets each session)
1. Docker Desktop running + Ollama running
2. `cloudflared tunnel --url http://localhost:5678` → copy the new address
3. Paste into `docker-compose.yml` `WEBHOOK_URL=` line
4. `docker compose up -d`
5. Unpublish → republish the Router workflow
...
docs/BUILD_JOURNEY.md                      # how it was built, decisions & war stories
```
...
- **Product concept, architecture & case study:** [Asmit Birthare](https://www.linkedin.com/in/asmit-birthare-501901248) — [asmit1624](https://github.com/asmit1624)
- **Engineering MVP:** Mehul Sharma ([mehulsharma-245](https://github.com/mehulsharma-245)) — built hands-on with AI pair-programming assistance, every architectural decision argued through and owned.

*Questions or feedback: [LinkedIn](https://www.linkedin.com/in/asmit-birthare-501901248)*
